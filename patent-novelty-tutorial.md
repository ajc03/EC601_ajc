# Repurposed Supervision: A Tutorial on Machine Learning for Patent Novelty Assessment

*EC601 Phase 1 Tutorial — for a masters student who has never worked with patent data*

---

## How to read this document

This tutorial teaches one research area: **computational assessment of patent novelty**, the task of deciding whether an invention described in a patent claim was already disclosed in an earlier document.

It is organised around a single argument, which is worth stating up front so you can disagree with it as you read:

> Almost every dataset in this field derives its labels from documents that were produced for a different purpose — examiner search reports, examiner opinions, patent membership, prosecution amendments. The labels are cheap and abundant because nobody made them for us. Understanding what that trade costs is the single most useful lens for reading this literature, and the largest remaining gap in it.

**Area alignment.** Patents are a domain, not one of the nine EC601 research areas. This tutorial sits in **Area 1: Machine Learning — Vision, Multimodal & Beyond**, specifically the bullets on *"VLM-powered applications with rigorous evaluation"* and *"retrieval-augmented pipelines: when should a system look things up versus rely on the model — and how do you measure the difference?"* Patent documents are the corpus; retrieval and evaluation are the research content.
...
**Verification status.** Three papers were read in full. Every factual claim attributed to them was checked against the paper text, not recalled. Claims of general legal background are flagged where they were *not* verified against primary sources. See §8.

---

## 1. Motivation: why anyone should care

### 1.1 What a patent is, in one paragraph

A patent gives its owner the right to exclude others from making, using, or selling an invention for a limited period. In exchange, the invention is published. The document has two halves that do different jobs: a **specification** that describes the invention in enough detail for someone skilled in the field to build it, and a set of **claims** — numbered sentences at the end that define the legal boundary of what is protected.

The distinction matters more than any other fact in this tutorial. The specification *describes*. Only the claims *delimit*. Risch et al. put it directly: the legally relevant definition of the invention is found in the independent claims, usually claim 1, which may be short and deliberately generalised in order to keep the scope of protection as broad as possible [1].

### 1.2 The task

To be patentable, an invention must be **new** over what came before. The body of everything publicly available before the relevant date is called **prior art**. A patent examiner must search it and decide whether the claimed invention is already disclosed there.

This is difficult for reasons that are specific and interesting:

- **The corpus is enormous.** Every published application and granted patent worldwide, in every language, plus scientific literature, product manuals, and public demonstrations.
...
### 1.3 Who is it for? (the question that determines everything else)
...
|---|---|---|---|
| **Decision** | Grant or reject this application | Can we invalidate this patent in litigation? | Can we ship this product? |
...
Risch et al. state the examiner case explicitly: only one retrieved "X" or "Y" document is enough to refuse claim 1, and for that reason the search task is focused more on precision than on recall [1].

**This is worth pausing on, because it is easy to over-generalise.** It is tempting to say "patent search is a precision-focused task." That is false as a general statement. Precision-dominance is a property of *the examiner's role and stopping rule*, not of patent search. Flip to the litigator and the asymmetry inverts completely: a missed reference is the whole case. Any paper you read is implicitly serving one of these users, and its metrics only make sense once you know which.

### 1.4 Real applications that exist today
...
- **Corporate IP clearance.** Large technology companies run continuous FTO monitoring. Bosch — whose research lab produced one of the three papers here [3] — is exactly such an organisation, which tells you something about why this research is funded.
...
### 1.5 Why it is a good research area for a semester

The data is free and public: EPO bulk full-text and Publication Server, the EPO Register, USPTO bulk downloads, PatentsView. No IRB, no data-access negotiation. The evaluation is hard in ways that are legible, and — as this tutorial will argue — the open problems do not all require large compute.
...
**Q1.3 (short answer).** A paper reports that its prior-art retrieval system achieves high precision and modest recall, and concludes that it is "suitable for deployment." What single question must you ask before accepting that conclusion?

<details>
<summary><b>Answers to Quiz 1</b></summary>

**Q1.1 — (c).** The claims, and specifically the independent claims. The specification teaches how to build the invention; the claims define what is owned. Dependent claims add optional features and depend on an independent claim [1].

**Q1.2 — (b).** A missed patent means shipping a product that infringes, with injunction and damages exposure. A false positive costs attorney time. The asymmetry is severe and points the opposite way from the examiner's.

**Q1.3.** *Who is the user?* High precision with modest recall is appropriate for an examiner, who stops at the first killer reference, and dangerous for an invalidity or FTO searcher, who needs completeness. The metric cannot be evaluated without the role.
...
A claim is a single sentence that defines the invention as a combination of **features** — each feature being a component, step, or characteristic [3]. Knappich et al. give the example of a claim about a text classification method, whose features might be: converting text into tokens; applying a neural network to the tokens; mapping embeddings into the label space [3].

Two structural facts follow:

**Independent versus dependent.** An independent claim stands alone. A dependent claim refers back to another claim and narrows it — "2. The system according to claim 1, wherein the light source is an OLED." A patent may have several independent claims, for example a system claim 1 and a method claim 15 [1].
...
Read that carefully. A prior art document that discloses nine of ten features destroys nothing. Adding features to a claim makes it *easier* to be novel and *narrower* in protection — which is precisely why applicants add limitations when an examiner objects, a fact that will come back as a serious methodological problem in §4.3.
...
**Every dataset in this tutorial is built from artefacts of this workflow.** Nobody in it was ever asked to label data for machine learning.

### 2.3 Novelty versus obviousness

Novelty (does *one* document disclose everything?) is distinct from obviousness or inventive step (would the combination have been obvious to a skilled person?). Novelty is the more tractable of the two: the single-document requirement gives it a crisp structure. Obviousness requires judging what a hypothetical skilled person would have found motivating to combine.

Every dataset discussed here targets **novelty**, and each excludes obviousness explicitly — Risch et al. drop "Y" citations entirely [1], and Knappich et al. filter out rejections based on lack of inventive step [3]. This is a sensible simplification and also a real limitation, since a great deal of actual examination turns on obviousness.

> **Verification note.** In the US these correspond to 35 U.S.C. §102 and §103; in the European system, to the EPC's novelty and inventive-step requirements. I have *not* verified the statutory text or article numbers against primary sources for this document. Do not cite section numbers from this tutorial — check the statute and the MPEP or EPO Guidelines directly.

### 2.4 Gold labels versus proxy labels

A **gold label** is produced by someone qualified to judge the task, for the task. A **proxy label** is a cheaper correlated signal used in its place.
...
None of these are wrong. All of them are *approximations whose error rate is unknown*. Two specific failure modes recur:

**Unreliable negatives.** Knappich et al. state it plainly: while cited passages reliably indicate disclosure, the absence of a citation does not imply non-disclosure — negatives may not be true negatives [3]. An examiner who found one killer passage had no reason to keep listing others.
...
A **spurious correlation** (or shortcut) is a feature that predicts the label without being causally related to the task. If claims that were rejected are systematically shorter than claims that were granted, a model can score well on "is this claim novel?" by counting words — never looking at the prior art at all.

This is not hypothetical here. It is the central finding of §4.3, and it is why the field's headline numbers need reading with care.
...
**Q2.2 — (c).** Applicants typically add limitations to overcome novelty objections [3]. The claim becomes narrower and therefore easier to distinguish from prior art — the classic trade of scope for grant.

**Q2.3 — (b).** The absence of a citation does not imply non-disclosure; an examiner stops once the point is made [3].

**Q2.4.** An "A" document was retrieved by an expert searching the same technical territory, so it is topically adjacent by construction — a hard negative. A model therefore cannot succeed by subject-matter similarity alone; it must judge whether the passage actually discloses the claimed combination, which is much closer to the legal task.

</details>

---

## 3. The retrieval machinery, briefly

You do not need deep IR background, but you need vocabulary to read the results tables.

### 3.1 Ways to match text

**Lexical matching (BM25, ROUGE-L overlap).** Score documents by shared terms, weighting rare terms more heavily and normalising for length. Fast, interpretable, and stubbornly hard to beat. Its weakness is exactly the patent vocabulary problem: it cannot match "elongate conductive member" to "wire."
...
1. **The denominator.** "Rank 1.42" sounds excellent until you learn it is among 8 in-batch candidates rather than a full corpus [1]. Always find the candidate-pool size.
...
3. **The baseline that shouldn't work.** If a model with no access to the evidence scores well, the benchmark has a shortcut [3].
...
**Q3.2 — (a).** With rank 0 as first position, 1.42 means roughly second-to-third out of eight [1]. It is a training diagnostic, not a retrieval result. Reporting it without the batch size would badly mislead.

**Q3.3.** Soft metrics give partial credit via textual overlap. Reporting them signals that the authors do not trust their negatives — that an uncited passage may still be relevant, so a hard-scored "false positive" may in fact be correct [3]. It is an honest admission that the ground truth has holes.

</details>

---

## 4. State of the art: three papers
...
**How the labels were made.** This is the important part. The authors did not annotate anything. They took EPO search reports — available in bulk from 2012 onward — parsed the citation entries, and resolved claim and paragraph numbers to their text. "X" citations became positives, "A" citations became negatives. "Y" citations were excluded, on the grounds that they seemed too close to "X" in semantic relevance to generate a good training signal [1].

**Scale, and what it means.** 6,259,703 samples — but only 297,147 distinct claim texts and 31,238 distinct applications [1]. That is roughly 21 samples per claim. The sample count wildly overstates independent information. The authors provide a stricter variant with exactly one X and one A per claim, which collapses the data to 25,340 samples [1]. That number, not six million, is the honest measure of independent signal.
...
**Results.** A fine-tuned BERT text-pair classifier reached **54%** accuracy on the balanced variant and 52% on the stricter one — barely above chance. Validation loss stopped improving after six epochs, so this is not undertraining. The authors are candid: it is not to their surprise that the task is hard, citing legal jargon and domain-specific language [1]. A DPR model reached an average in-batch rank of 1.42 out of 8, which they present modestly as useful for narrowing to a handful of candidates for a human expert [1].
...
c) Sample count overstates independent information; splits must group by claim
...
**Q4.3.** *Explanation A:* the task is genuinely hard — legal language and abstract feature-matching defeat a general-purpose encoder, which is what the authors conclude [1]. *Explanation B:* a share of the labels are wrong, because the X/A judgment was made per document per claim-group and then expanded into pairs. *To distinguish them:* have a qualified annotator re-label a random sample of pairs and measure agreement with the derived labels. High agreement points to task difficulty; low agreement points to label noise. Nobody has published this.
...
**Why a vision paper belongs here.** Patent drawings often carry information more accessible than the text, yet retrieval in this domain relies primarily on text and ignores the drawings [2]. If your prior art search is text-only, you are blind to a channel examiners actually use.
...
**The scope restriction you must not miss.** Design patents capture the visual characteristics of an object, so their figures are object depictions, whereas utility patents contain flowcharts, plots, mathematical expressions, and text-heavy mechanical diagrams [2]. DeepPatent is therefore a clean vision benchmark and a **poor proxy for utility-patent prior art** — and utility patents are where essentially all electrical and computing subject matter lives.
...
**Four results worth internalising.**

1. **ImageNet pretraining beats in-domain self-supervision.** A RotNet model self-supervised on the patent data itself reached 0.169 mAP against 0.291 for ImageNet weights, despite the ImageNet network never having seen a patent [2]. A useful corrective to the assumption that in-domain pretraining always wins.
2. **Sketch data actively hurts.** Fine-tuning on the Sketchy sketch dataset dropped mAP to 0.229 — *below* the plain ImageNet baseline [2]. Patent drawings and free-hand sketches look similar and are not the same domain. The supporting statistic is elegant: median connected components are 705 in DeepPatent against 2 in Sketchy, while ImageNet photo edge maps sit at 673 [2]. Structurally, patent drawings resemble photographs more than sketches.
...
**Naming hazard.** "DeepPatent" also names an unrelated 2018 text classification model, which this paper cites in its own reference list [2]. Always qualify: *the DeepPatent drawing dataset (Kucer et al., 2022)*.
...
b) Design patent drawings are object depictions; utility figures include flowcharts, plots, and equations
...
The comparison between the two is the single most useful number in this literature:

> **Only 19% of the passages cited in the ESR are also cited in the ESOP** [3].

Two causes: ESR citations are aggregated over multiple claims, losing which passage relates to which claim; and examiners merge ranges for brevity, citing paragraphs 10–20 instead of 10–13, 16–18, 20 [3]. ESOPs are the superior supervision at the cost of expensive PDF parsing [3].

Note what this does to §4.1. PatentMatch is built on ESRs. This is direct evidence that its supervision is coarse — and a plausible partial explanation for the 54% baseline.
...
**And the encouraging result:** LLM workflows show only minor accuracy loss or even gains on the adversarial subset, and show low agreement with the shortcut classifier — negative Cohen's kappa on the adversarial set [3]. Zero-shot LLMs are not riding these shortcuts, whereas fine-tuned classifiers are.
...
**Q4.10.** Any three of: (i) train a classifier that sees only the query and *not* the evidence — if it performs well, you have a shortcut; (ii) check for length or formatting differences between classes and stratify; (iii) strip artefacts introduced by the document lifecycle, such as reference numerals; (iv) build an adversarial subset from the shortcut model's errors as a diagnostic; (v) run a contamination check against indexed pretraining corpora; (vi) split by time or by group rather than randomly; (vii) establish a human expert baseline and inter-annotator agreement. Item (vii) is the one nobody has done.

</details>

---

## 5. Synthesis: the through-line
...
**1. Free labels are never free.** Each dataset bought scale by repurposing an artefact made for another purpose. Each inherited a specific distortion: coarse aggregation, same-patent relevance, prosecution-amendment proxies. The distortion is not a flaw in the work — it is the price, and the mature move is to measure it. The 19% ESR/ESOP figure [3] is the first time anyone in this line put a number on it.
...
**3. Every number needs its denominator.** Rank 1.42 out of 8 [1]. mAP 0.376 against a 38K database, 0.262 against 350K [2]. 75.2% on a length-stratified, numeral-stripped, four-CPC-class, granted-applications-only sample [3]. Strip the conditions and all three become misleading.

---

## 6. What is still open

**1. There is no human baseline.** Not one of these papers reports how well trained experts do on its task, or how much they agree with each other. Knappich et al. name this explicitly as future work [3]. Every reported number therefore floats without a ceiling: nobody knows whether 27.4 F1 on feature-level passage retrieval is poor or near the limit of what examiner agreement would support. **This is the largest and most tractable gap in the area, and it requires domain expertise rather than compute.**

**2. The disclosure-judgment bottleneck.** The oracle result [3] says retrieval is solved enough that it is no longer the limiter. What fails is deciding whether a passage discloses a feature. That is an abstract reasoning problem with a legal standard attached, and it is wide open.
...
## 7. How to read a paper in this area

A checklist distilled from the three papers. Apply it to the next one you read.

1. **What is the supervision source, and what was it originally for?** If the answer is a document produced by an examiner or by a legal process, ask what was lost in translation.
2. **What is the label granularity, and does it match the original judgment?** Expanding a document-level decision into pair-level labels manufactures precision that was never there.
3. **Are the negatives trustworthy?** Absence of a citation is not evidence of absence [3].
4. **Can the benchmark be beaten without the evidence?** If nobody ran that experiment, run it yourself.
5. **What is the denominator?** Candidate pool size, database size, sample restrictions.
6. **Is the split temporal or grouped?** Random splits leak across patent families.
7. **Is there a human ceiling?** Usually not. Note its absence when interpreting any score.
8. **Who is the user, and does the metric follow from their error costs?**
...
b) Labels repurposed from artefacts created for another purpose
...
**Q5.2 — (c).** Scores float without a ceiling. If trained examiners agreed with each other only 70% of the time on feature-level disclosure, a model at 65% would be nearly saturated rather than failing — and the field would be optimising noise. Knappich et al. flag this as open [3].
...
Listed for transparency. **Bibliographic details were verified; the contents were not read**, and no claim in this tutorial rests on them.
...
## Appendix: known limitations of this tutorial

Stated plainly, because the argument of §5 applies to this document too.

1. **Legal background is unverified.** Statutory references (§102/§103, EPC articles) were deliberately kept out of the body text. Verify against the statute and MPEP or EPO Guidelines before reusing.
2. **Three papers is a narrow base.** They were chosen to span text, vision, and the modern LLM era, but they are not a systematic survey. Surveys [4] and [5] would broaden it.
3. **Two of three datasets are EPO-derived.** Conclusions about supervision quality may not transfer to USPTO or other offices.
4. **The synthesis is an argument, not a finding.** The claim that repurposed supervision is *the* organising weakness of the field is the author's framing. It is supported by these three papers; it has not been tested against the field as a whole.
